...
export function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  ...
  const date = formatDate(testimonial.createdDateTime, lang === "no" ? "nb-NO" : lang);
  ...
  const content = (
    <blockquote className="testimonial-card flex min-h-[285px] h-full flex-col border border-border bg-card p-7 shadow-[0_18px_42px_-34px_rgba(43,30,22,0.5)] md:p-8">
      ...
      <p className="mt-2 flex-1 text-sm italic leading-7 text-foreground">{tx(testimonial.quote)}</p>
          ...
          <div className="min-w-0">
            ...
            {isTraderaReview ? (
              <div className="mt-1 flex items-center gap-1.5 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
                ...
                <span>{tx("Tradera buyer")}</span>
              </div>
            ) : <div className="mt-1 text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{tx(testimonial.descriptor)}</div>}
          </div>
  ...
  return testimonial.traderaUrl ? (
    <a href={testimonial.traderaUrl} target="_blank" rel="noreferrer" className="block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-sand" aria-label={tx("See review from {name} on Tradera", {name: testimonial.name})}>
      {content}
    </a>
  ) : content;
}
